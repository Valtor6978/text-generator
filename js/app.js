...
// Function For Add Paragraphs To DOM =>

function paragraphHandler (event) {
  event.preventDefault()

  let inputElemValue = inputElem.value
  let random = Math.floor(Math.random() * 10)

  if (inputElemValue < 0 || inputElemValue > 9 || inputElemValue === "" || isNaN(inputElemValue)) {
    result.innerHTML = dummyText[random]
  } else {
    let sliceText = dummyText.slice(0 , inputElemValue)
    let final = sliceText.map(function (x) {
      return "<p>"+ x +"</p>"  
    }).join("")
...
btn.addEventListener("click" , paragraphHandler)
